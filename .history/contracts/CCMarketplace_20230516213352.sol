...
contract CCMarketplace {
    ...
    //This mapping maps artists' addresses to allowed to upload NFTs or not 
    ...
    function approveArtist(address payable _artistAddress) public  {
        require (owner == msg.sender, "Only owner can update available artists");
        approvedArtists[_artistAddress] = true;
    }

    function revokeArtist(address payable _artistAddress) public {
        require (owner == msg.sender, "Only owner can update available artists");
        approvedArtists[_artistAddress] = false;
    }

    function addCharity(address payable _charityAddress, string memory _charityName) public { 
    ...
    function createListedToken(address token, uint256 tokenId, uint256 charityId, uint256 price) public {
        ...
        require(charityId >= currentcharityId, "charity does not exist"); 

        //require that artist has permission to create listing
        require(approvedArtists[msg.sender] == true, "must be approved to create listing");
    ...
    function executeSale(uint256 tokenId) public payable {
        ...
        address token = idToListedToken[tokenId].token;
        ...
        // Remove the token from the marketplace listings
        //**********todo */
        ...
        //Transfer the proceeds from the sale to the charity associated with NFT
